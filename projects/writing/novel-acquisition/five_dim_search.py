...
五维资料库一键检索脚本 v1.0
用法:
    python3 five_dim_search.py "关键词1,关键词2,关键词3" [--dim scenes,psychology,characters] [--top 10] [--novel 蛊真人]
    python3 five_dim_search.py --trait "开朗" [--dim all] [--top 15]
...
def search_five_dimensions(keywords: list[str], dimensions: list[str] = None, 
    ...
    keywords: 搜索关键词列表
    dimensions: 限定维度；None=全部。支持旧五维与 method_* 创作方法维度。
    top_n: 返回top N结果
    novel_filter: 限定某本小说
    """
    search_dims = dimensions if dimensions else None

    def _dim_matches(name: str) -> bool:
        if not search_dims:
            return True
        for item in search_dims:
            item = (item or "").strip()
    ...
    for novel_name, data in db.items():
        ...
        for anchor_info in anchors:
            ...
            for dim_name, entries in dims.items():
                if not _dim_matches(dim_name):
                    continue
                for entry in entries:
                    # twists/intelligence 是 llm_analysis 格式
                    text = entry.get("text", "") or entry.get("llm_analysis", "")
                    context = entry.get("context", "")
                    method_hint = " ".join(str(entry.get(key, "")) for key in ("method_label", "method_use"))
                    full_text = text + " " + context + " " + method_hint
...
def main():
    ...
    parser.add_argument("--dim", default="all", help="维度过滤：scenes,psychology,characters,twists,intelligence,method_* 或 all")
