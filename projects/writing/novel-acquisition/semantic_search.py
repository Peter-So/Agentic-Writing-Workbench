...
用法：
    python3 semantic_search.py "在饭堂吃饭被人注视"
    python3 semantic_search.py "少年被混混欺负后沉默" --top 10
    python3 semantic_search.py "师生关系，老师关心贫困学生" --dim psychology
    python3 semantic_search.py --build   # 重建索引（锚点更新后执行）
...
def load_all_segments():
    ...
    # Source 3: 项目产出降级语料 outputs_corpus/confirmed_outputs.json
    #   无 sidecar 时，用户确认的正文/摘要/设定落到这里，纳入 TF-IDF 重建供语义召回。
    outputs_corpus = BASE_DIR / "outputs_corpus" / "confirmed_outputs.json"
    if outputs_corpus.exists():
        try:
            records = json.loads(outputs_corpus.read_text(encoding="utf-8"))
        except Exception:
            records = []
...
def semantic_search(query, index_data, top_n=10, dim_filter=None, novel_filter=None):
    ...
    def _dim_matches(name: str, filters: list[str] | None) -> bool:
        if not filters:
            return True
        for item in filters:
            item = (item or "").strip()
    ...
    if dim_filter or novel_filter:
        for i, meta in enumerate(metadata):
            if dim_filter and not _dim_matches(str(meta["dimension"]), dim_filter):
                scores[i] = 0
...
def main():
    ...
    parser.add_argument('--dim', type=str, help='限定维度(scenes,psychology,characters,twists,intelligence,method_* 等)')
