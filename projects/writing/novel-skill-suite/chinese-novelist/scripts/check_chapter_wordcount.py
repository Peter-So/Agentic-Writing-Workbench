...
def check_chapter(file_path: str, min_words: int = 4000) -> dict:
    """检查单个章节的字数"""
    path = Path(file_path)
...
def check_all_chapters(directory: str, pattern: str = '第*.md', min_words: int = 4000) -> list:
    """检查目录下所有符合模式的章节文件"""
...
def print_results(results: list, min_words: int = 4000):
    """打印检查结果"""
    if not results:
        print('没有找到章节文件')
        return
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print('  python check_chapter_wordcount.py novels/故事/第01章.md 4000')
        print('  python check_chapter_wordcount.py --all novels/故事')
        print('  python check_chapter_wordcount.py --all novels/故事 4000')
        return
    ...
    if sys.argv[1] == '--all':
        ...
        min_words = int(sys.argv[3]) if len(sys.argv) > 3 else 4000
        results = check_all_chapters(directory, min_words=min_words)
        print_results(results, min_words)
    else:
        file_path = sys.argv[1]
        min_words = int(sys.argv[2]) if len(sys.argv) > 2 else 4000
