...
## Agentic-Writing-Workbench-v0.1.10

本版同步 writing 工作项目在 v0.1.9 之后的图片预览能力和创作流程文档，并完成电影脚本视觉链路的文件级复核。电影脚本、分镜提示词、生图和技能库此前已经进入公开仓库，本次保留公开库中横线命名及空白图片 API 默认配置，不做破坏性覆盖。

### 新增

- Web 文件树支持 `.png`、`.jpg`、`.jpeg`、`.webp`、`.gif` 图片文件只读预览。
- 新增 `/api/writing/file-media` 安全媒体接口；沿用项目路径校验，只返回允许范围内的图片文件。
- 文件编辑器新增自适应图片画布，显示 MIME 类型和文件大小；图片不进入文本保存或改写接口。
- README 新增小说创作阶段推进条件，以及右侧“门禁、轨迹、经验”三项功能的真实数据流和人工操作边界。
- 新增两张手绘看板式流程说明图，并补充 Writing 详细设计文档。

### 对齐确认

- 小说当前阶段仍以结构 Wiki 路由文件和章节确认记录为事实来源；页面刷新状态后显示第一个未完成阶段。
- 电影脚本的概念、角色、节拍、剧本、分镜、生词、生图链路已在公开库中，无需重复迁移。
- 保留公开库 `app/static-writing/`、中横线技能文件名、`#upgradeBtn`、`app_upgrade` 状态流和 `/api/app-upgrade/*` 路由。
- 门禁建议与轨迹保持只读；经验只有在用户点击“采纳首条”后才写入 lessons 和技能库。

### 修复

- 修复图片文件在文件树中可见但打开后提示“不支持预览”的问题。
- 阻止图片内容误入文本保存接口，避免按 UTF-8 文本读取或覆盖二进制文件。

## Agentic-Writing-Workbench-v0.1.9

本版补充 Web UI 主题切换能力，让用户可以在明亮、黑夜、护眼三种界面主题之间切换，并保持刷新和重启后的主题偏好。

### 新增

- 提问框操作区新增主题下拉框，提供 `明亮`、`黑夜`、`护眼` 三个选项。
- 主题偏好保存到浏览器本地存储，刷新页面和重启服务后保持不变。
- 页面加载前先恢复已保存主题，减少刷新时的明暗闪烁。

### 优化

- 主题样式改为 CSS 变量驱动，背景、卡片、输入框、消息、流程图和状态提示统一跟随主题切换。
- 主题控件右对齐显示在提问框操作区，保持紧凑宽度。
- 主题下拉框边框样式与参考小说工作台下拉框统一。

### 修复

- 修复切换主题后下拉框处于 focus 状态时边框样式变化、刷新后才恢复的问题。
- 同步公开库时继续保留 Web 更新入口、`app_upgrade` 状态流和 `/api/app-upgrade/*` 路由。

## Agentic-Writing-Workbench-v0.1.8

本版同步 writing 工作项目的网页 provider 抓取、任务状态恢复和用户确认闭环修复，重点解决网页模型回答复制、任务状态栏、定稿确认和公开库差异保留问题。

### 新增

- provider 回答卡片支持用户编辑，确认材料、继续融合、归档等后续动作会使用前端最新内容。
- 网页 provider 抓取增加剪贴板锁，千问、DeepSeek、豆包共用系统剪贴板时串行复制和读取。
- 中断任务提供继续当前任务和终止任务入口；终止会清理当前任务相关 pending/workflow 记忆。

### 优化

- 网页 provider 抓取统一点击页面回答区域提供的复制按钮；复制按钮必须位于本轮提问坐标之后，避免抓旧回答或提问 hover 复制。
- 移除短回答长度限制、短问期望答案特判、DOM 签名校验和 14 秒等待限制，短回答也能正常回填。
- 任务状态栏只展示主创作流程，打开网页、打开文件、打开流程图等非主流程动作不再污染状态栏。
- 任务计时统一为 `s`、`m`、`h` 简写，修复双 timer 争抢导致的闪烁和阶段状态错位。

### 修复

- 修复已进入定稿/用户确认阶段时，主对话框仍显示旧 provider 确认按钮的问题。
- 修复用户确认阶段被直接标记完成而没有提供确认按钮的问题。
- 修复网页模型聊天与创作模式 provider 抓取逻辑不一致的问题。
